...
from modules.auth.schemas import SignupSchema, LoginSchema, ResetPasswordSchema
from modules.auth.service import check_user_exists, add_user, get_user_by_email, update_user_password
...
async def reset_password(data: ResetPasswordSchema, db: Session = Depends(get_db)):
    user = get_user_by_email(db=db, user_email=data.email)
    if not user:
        raise HTTPException(status_code=404, detail="User with this email not found.")

    hashed_pass = get_password_hash(data.new_password)

    success = update_user_password(db=db, email=data.email, hashed_password=hashed_pass)
